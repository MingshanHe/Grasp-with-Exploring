...
class Logger():
    ...
    def __init__(self, continue_logging, logging_directory):
        ...
        self.cognition_directory = os.path.join(self.base_directory, 'data', 'cognition')
        ...
        if not os.path.exists(self.cognition_directory):
            os.makedirs(self.cognition_directory)
    ...
    def save_force_sensor_data(self, iteration, force_sensor_data):
        np.savetxt(os.path.join(self.force_sensor_data_directory, '%s.force_sensor_data.txt' % iteration), force_sensor_data, delimiter=' ')
    ...
    def save_cognition_data(self, iteration, cognition_data):
        np.savetxt(os.path.join(self.cognition_directory, '%06d.cognition.txt' % iteration), cognition_data, delimiter=' ')

    def save_cognition_images(self, iteration, cognition_image):
        cv2.imwrite(os.path.join(self.cognition_directory, '%06d.cognition.png' % (iteration)), cognition_image)
